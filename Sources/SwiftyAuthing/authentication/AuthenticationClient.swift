...
//    }
    ...
    /// 使用邮箱注册，此接口不要求用户对邮箱进行验证，用户注册之后 emailVerified 字段会为 false 。如果你希望邮箱未验证的用户不能进行登录，可以使用 pipeline 对此类请求进行拦截。
    ///
    public func registerByEmailWithResult(email: String, password: String, profile: RegisterProfile? = nil, forceLogin: Bool? = nil, generateToken: Bool? = nil, clientIp: String? = nil, completion: @escaping ((Result<GraphQLResult<RegisterByEmailMutation.Data>, Error>) -> Void)) {
        Network.shared.apollo.perform(mutation: RegisterByEmailMutation(email: email, password: Encryption.encrypt(password), profile: profile, forceLogin: forceLogin, generateToken: generateToken, clientIp: clientIp), queue: DispatchQueue.main) { result in
            switch result {
            case .failure(let error):
                print("Failure: \(error)")
    ...
    /// 使用用户名注册
    ///
    public func registerByUsernameWithResult(username: String, password: String, profile: RegisterProfile? = nil, forceLogin: Bool? = nil, generateToken: Bool? = nil, clientIp: String? = nil, completion: @escaping ((Result<GraphQLResult<RegisterByUsernameMutation.Data>, Error>) -> Void)) {
        Network.shared.apollo.perform(mutation: RegisterByUsernameMutation(username: username, password: Encryption.encrypt(password), profile: profile, forceLogin: forceLogin, generateToken: generateToken, clientIp: clientIp), queue: DispatchQueue.main) { result in
            switch result {
            case .failure(let error):
                print("Failure: \(error)")
    ...
    /// 使用手机号注册，你可以同时设置该账号的初始密码。发送短信的接口请见 sendSmsCode
    ///
    public func registerByPhoneCodeWithResult(phone: String, code: String, password: String, completion: @escaping ((Result<GraphQLResult<RegisterByPhoneCodeMutation.Data>, Error>) -> Void)) {
        Network.shared.apollo.perform(mutation: RegisterByPhoneCodeMutation(phone: phone, code: code, password: Encryption.encrypt(password)), queue: DispatchQueue.main) { result in
            completion(result)
        }
    }
    ...
    /// 使用手机号注册，你可以同时设置该账号的初始密码。发送短信的接口请见 sendSmsCode
    ///
    public func registerByPhoneCodeWithResult(phone: String, code: String, password: String, profile: RegisterProfile? = nil, forceLogin: Bool? = nil, generateToken: Bool? = nil, clientIp: String? = nil, completion: @escaping ((Result<GraphQLResult<RegisterByPhoneCodeMutation.Data>, Error>) -> Void)) {
        Network.shared.apollo.perform(mutation: RegisterByPhoneCodeMutation(phone: phone, code: code, password: Encryption.encrypt(password), profile: profile, forceLogin: forceLogin, generateToken: generateToken, clientIp: clientIp), queue: DispatchQueue.main) { result in
            switch result {
            case .failure(let error):
                print("Failure: \(error)")
    ...
    /// 使用邮箱登录，该接口默认不会限制未验证的邮箱进行登录，如果你希望邮箱未验证的用户不能进行登录，可以使用 pipeline 对此类请求进行拦截。如果你的用户池配置了登录失败检测，当同一 IP 下登录多次失败的时候会要求用户输入图形验证码（code 为 2000)。
    ///
    public func loginByEmailWithResult(email: String, password: String, autoRegister: Bool? = nil, captchaCode: String? = nil, clientIp: String? = nil, completion: @escaping ((Result<GraphQLResult<LoginByEmailMutation.Data>, Error>) -> Void)) {
        Network.shared.apollo.perform(mutation: LoginByEmailMutation(email: email, password: Encryption.encrypt(password), autoRegister: autoRegister, captchaCode: captchaCode, clientIp: clientIp), queue: DispatchQueue.main) { result in
            switch result {
            case .failure(let error):
                print("Failure: \(error)")
    ...
    /// 使用用户名登录
    ///
    public func loginByUsernameWithResult(username: String, password: String, autoRegister: Bool? = nil, captchaCode: String? = nil, clientIp: String? = nil, completion: @escaping ((Result<GraphQLResult<LoginByUsernameMutation.Data>, Error>) -> Void)) {
        Network.shared.apollo.perform(mutation: LoginByUsernameMutation(username: username, password: Encryption.encrypt(password), autoRegister: autoRegister, captchaCode: captchaCode, clientIp: clientIp), queue: DispatchQueue.main) { result in
            switch result {
            case .failure(let error):
                print("Failure: \(error)")
    ...
    /// 使用手机号验证码登录。
    ///
    public func loginByPhoneCodeWithResult(phone: String, code: String, clientIp: String, completion: @escaping ((Result<GraphQLResult<LoginByPhoneCodeMutation.Data>, Error>) -> Void)) {
        Network.shared.apollo.perform(mutation: LoginByPhoneCodeMutation(phone: phone, code: code, clientIp: clientIp), queue: DispatchQueue.main) { result in
            switch result {
            case .failure(let error):
                print("Failure: \(error)")
    ...
    /// 使用手机号密码登录
    ///
    public func loginByPhonePasswordWithResult(phone: String, password: String, captchaCode: String? = nil, autoRegister: Bool? = nil, clientIp: String? = nil, completion: @escaping ((Result<GraphQLResult<LoginByPhonePasswordMutation.Data>, Error>) -> Void)) {
        Network.shared.apollo.perform(mutation: LoginByPhonePasswordMutation(phone: phone, password: Encryption.encrypt(password), captchaCode: captchaCode, autoRegister: autoRegister, clientIp: clientIp), queue: DispatchQueue.main) { result in
            switch result {
            case .failure(let error):
                print("Failure: \(error)")
    ...
    /// 退出登录
    ///
    public func logoutWithResult(completion: @escaping ((Result<GraphQLResult<LogoutMutation.Data>, Error>) -> Void)) {
        let id = self.getUserId()
        Network.shared.apollo.perform(mutation: LogoutMutation(id: id, tokenExpiredAt: "0"), queue: DispatchQueue.main) { result in
            switch result {
            case .failure(let error):
